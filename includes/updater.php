...
final class RY_WEZI_Updater
{
    protected static $_instance = null;

    public static function instance(): RY_WEZI_Updater
    {
    ...
    {
        if ('RY ezPay Invoice for WooCommerce' !== $plugin_data['Name']) {
            return $update;
        }
    ...
    public static function modify_plugin_details($result, $action, $args)
    {
        ...
        if ($args->slug != 'ry-woocommerce-ezpay-invoice') {
            return $result;
        }

        $response = RY_WEZI_LinkServer::instance()->get_info();
        if (!empty($response)) {
