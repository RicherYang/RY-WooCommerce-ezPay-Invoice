...
$(function () {
    if ($('#RY_WEZI_get_mode').length) {
        $('#RY_WEZI_get_mode').on('change', function () {
            let $item = $('#RY_WEZI_skip_foreign_order').closest('tr');

            if ($(this).val() == 'manual') {
                $('#RY_WEZI_skip_foreign_order').closest('tr').hide();
            } else {
                $('#RY_WEZI_skip_foreign_order').closest('tr').show();
            }
        }).trigger('change');
    }
