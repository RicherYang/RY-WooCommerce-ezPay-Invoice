...
final class RY_WEZI_WC_Invoice extends RY_WEZI_Model
{
    ...
    {
        include_once RY_WEZI_PLUGIN_DIR . 'woocommerce/abstracts/abstract-ezpay.php';
        include_once RY_WEZI_PLUGIN_DIR . 'woocommerce/invoice-api.php';

        switch (RY_WEZI::get_option('get_mode')) {
            case 'auto_paid':
        ...
        if ('auto_cancell' === RY_WEZI::get_option('invalid_mode')) {
        ...
        if (is_admin()) {
            include_once RY_WEZI_PLUGIN_DIR . 'woocommerce/admin/ajax.php';
            RY_WEZI_WC_Admin_Ajax::instance();

            include_once RY_WEZI_PLUGIN_DIR . 'woocommerce/admin/invoice.php';
            RY_WEZI_WC_Admin_Invoice::instance();
        } else {
            add_filter('default_checkout_invoice_company_name', [$this, 'set_default_invoice_company_name']);
            if ('yes' === RY_WEZI::get_option('show_invoice_number', 'no')) {
    ...
    public function auto_get_invoice($order_ID)
    {
        ...
        $skip_shipping = apply_filters('ry_wezi_skip_autoget_invoice_shipping', []);
        ...
        if ('yes' === RY_WEZI::get_option('skip_foreign_order', 'no')) {
        ...
        if (! WC()->queue()->get_next(RY_WEZI::OPTION_PREFIX . 'auto_get_invoice', [$order_ID], 'ry-invoice')) {
            WC()->queue()->schedule_single(time() + MINUTE_IN_SECONDS * 2, RY_WEZI::OPTION_PREFIX . 'auto_get_invoice', [$order_ID], 'ry-invoice');
        }
    }
    ...
    public function auto_delete_invoice($order_ID)
    {
        ...
        if ($invoice_number) {
            if ('zero' == $invoice_number) {
                ...
                WC()->queue()->schedule_single(time() + MINUTE_IN_SECONDS * 2, RY_WEZI::OPTION_PREFIX . 'auto_invalid_invoice', [$order_ID], 'ry-invoice');
            }
        }
    }
